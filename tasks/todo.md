...
## 概要
ExpandableChatコンポーネントのhandleSendMessage関数にAI機能を統合し、ユーザーが入力したテキストから予定を抽出してカレンダーに自動登録する機能を実装します。

## 実装計画

### Phase 1: プロジェクト構造調査
- [ ] 既存のプロジェクト構造とAPIの使用状況を調査する
- [ ] AI API設定と環境変数を確認する  
- [ ] 予定データの型定義を確認・作成する

### Phase 2: AI予定抽出サービス実装
- [ ] AI予定抽出サービス関数を実装する
- [ ] OpenAI/Claude等のAI APIとの連携を設定
- [ ] 予定データの構造化ロジックを実装

### Phase 3: チャット機能統合
- [ ] handleSendMessage関数を更新してAI機能を統合する
- [ ] チャット画面に処理状況表示機能を追加
- [ ] エラーハンドリングとユーザー通知を実装
...
### Phase 1: プロジェクト構造調査（完了）
- [x] 既存のプロジェクト構造とAPIの使用状況を調査する
- [x] AI API設定と環境変数を確認する  
- [x] 予定データの型定義を確認・作成する

### Phase 2: AI予定抽出サービス実装（完了）
- [x] AI予定抽出サービス関数を実装する
- [x] OpenAI/Gemini両方のAI APIとの連携を設定
- [x] 予定データの構造化ロジックを実装

### Phase 3: チャット機能統合（完了）
- [x] handleSendMessage関数を更新してAI機能を統合する
- [x] チャット画面に処理状況表示機能を追加
- [x] エラーハンドリングとユーザー通知を実装

## 実装されたファイル

### 1. 型定義: `src/types/aiEvent.ts`
- AIExtractedEvent: AI抽出された予定のデータ構造
- AIEventExtractionResponse: AI APIからのレスポンス型
- AIServiceConfig: AIサービスの設定型

### 2. AIサービス: `src/services/aiEventExtractionService.ts`
- OpenAI GPT-4o-miniとGemini-1.5-flashに対応
- 自動的なAPIプロバイダー選択（設定された環境変数に基づく）
- テキストから予定情報を抽出するメイン関数
- 抽出データのバリデーションと正規化

### 3. チャット機能統合: `src/components/ExpandableChat.tsx`
- AI予定抽出機能をhandleSendMessage関数に統合
- リアルタイムチャット履歴表示
- 処理状況の視覚的フィードバック
- エラーハンドリングと成功通知
...
- **Gemini API**: Gemini-1.5-flash-latestモデルを使用
- 環境変数に基づく自動プロバイダー選択
- 構造化されたJSON応答の生成と解析

### データ処理
- テキストから予定情報を構造化データに変換
- 日付・時間の自動解析とバリデーション
- 信頼度スコアによる抽出品質評価
- EventContextとの完全統合
...
ユーザーが以下のようなテキストを入力すると：
- 「明日午後2時から会議」
- 「来週の火曜日に歯医者の予約」
- 「12月25日にクリスマスパーティー」

AIが自動的に予定を解析してカレンダーに追加されます。